'use client';

import { useEffect, useState } from 'react';
import { formatNumber } from '@/lib/utils';

interface DownloadsData {
  apple_downloads: number | null;
  google_play_downloads: number | null;
  total_downloads: number;
}

export default function AppDownloads() {
  const [data, setData] = useState<DownloadsData | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('/api/app-downloads')
      .then((res) => res.json())
      .then((result) => {
        setData(result.data ?? null);
      })
      .catch(() => setData(null))
      .finally(() => setLoading(false));
  }, []);

  if (loading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="card">
            <div className="loading h-8 w-24 mb-2"></div>
            <div className="loading h-4 w-32"></div>
          </div>
        ))}
      </div>
    );
  }

  const apple = data?.apple_downloads ?? null;
  const google = data?.google_play_downloads ?? null;
  const total = data?.total_downloads ?? 0;

  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div className="bg-gradient-to-br from-primary to-primary-dark rounded-xl p-6 shadow-lg border border-primary/20">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-white/90 uppercase tracking-wide">Apple App Store</h3>
          <svg className="w-8 h-8 text-white/80" fill="currentColor" viewBox="0 0 24 24">
            <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
          </svg>
        </div>
        <p className="text-4xl font-bold text-white mb-1">
          {apple !== null ? formatNumber(apple) : '—'}
        </p>
        <p className="text-sm text-white/80">Downloads</p>
      </div>

      <div className="bg-gradient-to-br from-secondary to-secondary-dark rounded-xl p-6 shadow-lg border border-secondary/20">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-white/90 uppercase tracking-wide">Google Play</h3>
          <svg className="w-8 h-8 text-white/80" fill="currentColor" viewBox="0 0 24 24">
            <path d="M3.609 1.814L13.792 12 3.61 22.186a.996.996 0 01-.61-.92V2.734a1 1 0 01.609-.92zm10.89 10.893l2.302 2.302-10.937 6.333 8.635-8.635zm3.199-3.198l2.807 1.626a1 1 0 010 1.73l-2.808 1.626L12.001 12l5.697-3.491zM5.864 2.658L16.802 8.99l-2.302 2.302-8.636-8.634z" />
          </svg>
        </div>
        <p className="text-4xl font-bold text-white mb-1">
          {google !== null ? formatNumber(google) : '—'}
        </p>
        <p className="text-sm text-white/80">Downloads</p>
      </div>

      <div className="bg-gradient-to-br from-primary/90 to-primary-dark/90 rounded-xl p-6 shadow-lg border border-primary/20">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-white/90 uppercase tracking-wide">Total Downloads</h3>
          <svg className="w-8 h-8 text-white/80" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
          </svg>
        </div>
        <p className="text-4xl font-bold text-white mb-1">{formatNumber(total)}</p>
        <p className="text-sm text-white/80">Apple + Google Play</p>
      </div>
    </div>
  );
}
