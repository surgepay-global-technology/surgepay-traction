...
export default function TransactionStats() {
  ...
  const totalTransactions = stats.reduce((sum, s) => sum + s.total_transaction_count, 0);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      {stats.map((stat, index) => (
        <div key={stat.currency} className={`${index === 0 ? 'bg-gradient-to-br from-secondary to-secondary-dark' : 'bg-gradient-to-br from-primary/90 to-primary-dark/90'} rounded-xl p-6 shadow-lg border ${index === 0 ? 'border-secondary/20' : 'border-primary/20'}`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-white/90 uppercase tracking-wide">
              {stat.currency} Volume
            </h3>
            <svg className="w-8 h-8 text-white/80" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <p className="text-sm text-white/80">
            {formatNumber(stat.total_transaction_count)} transactions
          </p>
        </div>
      ))}
    </div>
